...
def find_best_pruning_m(train_dataset: np.array, m_choices, num_folds=5):
    ...
    for i, m in enumerate(m_choices):
        ...
        accuracies.append([])
        counter = 1
        examples, labels, _, _ = get_dataset_split(train_dataset, test_dataset, target_attribute)
        kfolds = KFold(n_splits=num_folds, random_state=ID_KEY, shuffle=True)
        for train_index, test_index in kfolds.split(train_dataset):
            print('m = ', m, 'fold number is ', counter)
            counter += 1
            train, train_labels = examples[train_index], labels[train_index]
            test, test_labels = examples[test_index], labels[test_index]
            model.fit(train, train_labels)
            x_test_result = model.predict(test)
            labels_set = set()
            for k in train_labels:
                labels_set.add(k)
            labels_list = list(labels_set)
            bool_x_test = np.zeros(test_labels.shape[0])
            bool_y_test = np.zeros(test_labels.shape[0])
            for k in range(len(bool_x_test)):
                if x_test_result[k] == labels_list[0]:
                    bool_x_test[k] = 1
                if test_labels[k] == labels_list[0]:
                    bool_y_test[k] = 1
            acc = accuracy(bool_x_test, bool_y_test)
...
def basic_experiment(x_train, y_train, x_test, y_test, formatted_print=False):
    ...
    x_test_result = id3.predict(x_test)
    labels_set = set()
    for i in y_train:
        labels_set.add(i)
    labels_list = list(labels_set)
    bool_x = np.zeros(y_test.shape[0])
    bool_y = np.zeros(y_test.shape[0])
    for i in range(len(bool_x)):
        if x_test_result[i] == labels_list[0]:
            bool_x[i] = 1
        if y_test[i] == labels_list[0]:
            bool_y[i] = 1
    acc = accuracy(bool_x, bool_y)
    # ========================
...
def best_m_test(x_train, y_train, x_test, y_test, min_for_pruning):
    ...
    for i in y_train:
        labels_set.add(i)
    labels_list = list(labels_set)
    boolean_x_test = np.zeros(y_test.shape[0])
    boolean_y_test = np.zeros(y_test.shape[0])
    for i in range(len(boolean_x_test)):
        if x_test_result[i] == labels_list[0]:
            boolean_x_test[i] = 1
        if y_test[i] == labels_list[0]:
            boolean_y_test[i] = 1
    acc = accuracy(boolean_x_test, boolean_y_test)
    # ========================
